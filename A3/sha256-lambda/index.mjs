...
export const handler = async (event) => {
    ...
    let hash;
    try {
        hash = crypto.createHash('sha256').update(event.value).digest('hex');
        console.log(hash);
    } catch (error) {
        return {
            statusCode: 400,
    ...
    const data = {
        banner: "B00981520",
        result: hash,
        arn: "arn:aws:lambda:us-east-1:835573539094:function:sha256-lambda",
        action: "sha256",
        value: event.value
    };
    console.log(data)
    ...
    return {
        statusCode: 200,
        body: JSON.stringify('Hello sha256 Lambda!'),
    };
};
